...
**The window's control is a toolbar popover, not a settings field.** `src/bar-chart-overlay/bar-chart-rows-panel.js` puts the presets, a custom count, an All, and the Top/Bottom switch one click from the chart — the settings modal holds only what you set once and leave (titles, axis labels, decimals), and how much of the ranking is on screen is the question the chart raises about itself the moment it truncates. The trigger doubles as the readout, labelled `Rows: 40 of 500`. `row_limit` therefore has THREE states and they must not be collapsed: absent is the default, an explicit `null` is every row, a positive integer is that many. "All" writing today's row count instead of `null` would pin a saved view to the size its data happened to be. And any new popover in a chart overlay must stop Escape on the CAPTURE phase itself — the overlay closes the whole chart on a document-level Escape, and the settings modal's fix does not transfer.
...
**`reverse_category_axis` cannot be passed through to Highcharts, because its default is not the same in both orientations.** An inverted chart already reverses its category axis by default, so that categories read downward from the top rather than up from the origin. The same user request — run the ranking the other way — is therefore `reversed: true` in a vertical chart and `reversed: false` in a horizontal one, and passing the flag straight through leaves the horizontal case doing the opposite of what was asked while the vertical case looks correct. One shared field rather than one per orientation: "the ranking runs the other way" is the same request in both, and it survives an orientation flip. It is also NOT the Rows panel's Top / Bottom — `rank_window` picks which rows survive the cap, this picks where the survivors are drawn.

**Orientation is a different CHART, so the overlay keys the mount to it.** `highcharts-react-official` applies any changed options object with `chart.update(options, true, true)`, so without a React `key` Highcharts transitions the LIVE bars across the inversion and the whole plot appears to rotate — each bar interpolating from tall-and-thin to short-and-wide, passing through square. Keying the mount to orientation throws the old chart away, and the replacement runs its own build animation: a column series grows up from the zero baseline, a bar series out from the side. Every other option change keeps the update path, where morphing is the right behaviour. **The growth is layered on in the overlay, not put in the options builder, and that placement is load-bearing**: the builder is a pure function whose output is rendered directly by harnesses that read bar, logo and label GEOMETRY back out of the SVG, and with animation in it those reads race the transition and return mid-flight numbers — a silent wrong answer, not an error. `plotOptions.series.animation: false` in the builder stays.
...
**Column header quick filter:** `src/column-quick-filter/` renders inside the header's existing Popper and writes ordinary `where` rows — no new operator, no new table-state field, no parallel filter store, so a view written from it round-trips against an un-bumped server. A range is two rows (`>=` plus `<=`); `BETWEEN` exists in none of the five allowlists and `src/validators/table-state-validator.mjs` forbids array values on comparison operators, so it would need a whole new operator class to express what two rows already express.

The projection is a pure lens, `src/utils/column-filter-state.js` (`test/column-filter-state.spec.mjs`), and it is separate from the component because the index contract is what regresses silently. **Indexes into `where` are load-bearing for four consumers, not two**: `build_where_column_definitions` is positionally aligned, `table-filter-controls` bulk-removes by `selected_where_indexes`, and league's `data-view-notices.mjs` carries `filter_index` AND persists notice dismissals as `` `${code}:${filter_index}` `` — so a shifted index silently resurrects a dismissed notice or pre-dismisses a new one. `apply_column_filter_state` therefore rewrites at a held index, appends only for a genuinely new row, and never touches an index its descriptor did not claim.

**The matching key is `column_id` plus `column_index` and deliberately NOT `params`.** `table.js` `remove_disabled_params` rewrites `item.params` in place on every commit whenever `row_axes` is non-empty, so a params-bearing key stops matching the row it just wrote: the panel reads blank and the next edit appends a contradictory duplicate. Rows the lens CREATES still carry `params` so they target the right column instance; nothing ever matches on them. `test/column-filter-state.spec.mjs` has the discriminating case, which is an apply round trip rather than a resolve — a resolve assertion cannot fail, because resolve takes no params argument at all.

**What the panel cannot represent it reports and never repairs.** A second row tying on the same key and slot, or an operator outside the vocabulary, joins `unrepresentable_where_indexes` and is counted on screen with a link to the side panel. The side panel is strictly more expressive and `enable_duplicate_column_ids` makes multi-row-per-column a producible state, so silently rewriting a filter built there would be data loss. `TableQuickFilter` resolves ownership through the same lens so the two surfaces cannot disagree about which row a duplicated column owns.

**`undefined` is a reachable `data_type`, not a defensive branch.** A consumer can name a constant that does not exist (league did: `TABLE_DATA_TYPES.PERCENT`), and that column must still get a menu rather than losing its Filter entry. `JSON`, `BINARY_UUID`, `RANGE`, `OBJECT_PRESET` and `undefined` all render the advanced-filter link alone — note `RANGE` has no `DATA_TYPE_OPERATORS` entry at all.

Two things the copy decides. **TEXT leads with Equals and offers Contains second**, because most TEXT columns in practice are external-system identifier fields where a substring match answers no question anyone asked. And **`IS NULL` is labelled "Is empty"** to match `src/constants.mjs` — the `IS EMPTY` / `IS NOT EMPTY` pair in `schema/base/table-operators.json` is a trap, present in neither `TABLE_OPERATORS` nor the server enum.

**A sort clause names a column OCCURRENCE, so every edit to `columns` must carry the sort with it.** The pair is `column_id` plus `column_index`, the ordinal among the columns sharing that id, and the server resolves a clause by the same pair — an ordinal that matches nothing resolves to nothing and the sort is dropped from the query with no error and no indicator, since the header's sort arrow is keyed to a rendered column. Matching on `column_id` alone is wrong in BOTH directions once `enable_duplicate_column_ids` makes duplicates producible: it drops a sort belonging to a surviving twin, and it leaves a stale ordinal on the twin that survives a removal. Route every removal, reorder, insertion and replacement through `src/utils/remap-sort-for-column-change.js` (`test/remap-sort-for-column-change.spec.mjs`), which takes the previous columns, the next columns, and a `next_to_previous_index` describing where each entry came from. A clause that did not resolve BEFORE the edit is deliberately left untouched: something else orphaned it, and deleting it here would turn an unrelated removal into data loss for a sort that is still restorable by re-adding the column.

**A `Popper` must render INSIDE its `ClickAwayListener`'s wrapped node, and the check that catches a violation is a TAP rather than a click.** The listener decides "away" on the React tree, not the DOM — both arrangements portal to body — so a popper rendered as a sibling makes every click inside the menu an away-click. On desktop that is invisible: the away handler is a document `click` listener, and React dispatches the item's own `onClick` at the portal container first, so the action runs and the panel merely closes after it. On touch the default `touchEvent: 'onTouchEnd'` fires BEFORE the click, the item unmounts, and the click lands on nothing — the control is simply dead, with no error anywhere. `table-header` shipped that way and every entry in the column header menu was inert on mobile Safari (`test/table-header-touch-menu.spec.jsx` drives the real `touchend`-then-`click` sequence; a click-only test passes against the bug). `column-picker.js` inverts the nesting — listener inside the popper, wrapping the panel — which is equally sound, since then the panel is the wrapped node. Where the trigger cannot take a laid-out parent, group the two under a `display: contents` wrapper: `.cell` is a direct flex child of `.row` and positions sticky against the scroll container, and a real wrapper box breaks both.

**A nested popper stops auto-dismissing, so one-shot menu actions must close the menu themselves.** The sibling arrangement above closed the panel as a side effect of every interaction, which is also why it was never obvious that the quick-filter inputs could not have worked from it. Once nested, nothing closes on its own: `table-header`'s four sort handlers and its advanced-filter entry call an explicit close, while the scatter/bar-chart toggles and the quick filter deliberately stay open because they are read and adjusted in place.
